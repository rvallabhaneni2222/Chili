...
public abstract class AbstractHandleEntityDao<T extends AbstractHandleEntity> {
    ...
    @Transactional(readOnly = true)
    public AbstractHandleEntity findBySource(Long id, String sourceClassName) {
        TypedQuery<AbstractHandleEntity> query = getEntityManager().createQuery("from " + entityCls.getCanonicalName() + " where sourceEntityName=:sourceEntityNameParam and sourceEntityId=:sourceEntityIdParam", AbstractHandleEntity.class);
